...
class Api extends ResourceController
{
    ...
    public function polygon() {
        $db = db_connect();
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, ST_Area(geom, true) AS luas_m2, nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polygon WHERE deleted_at IS NULL");
    ...
    public function polyline() {
        $db = db_connect();
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, ST_Length(geom, true) AS panjang_m, nama, deskripsi, foto, created_at, updated_at FROM  tbl_data_polyline");
    ...
    public function one_polygon($id){
        $db = db_connect();
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, ST_Area(geom, true) AS luas_m2, nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polygon WHERE deleted_at IS NULL AND id =  $id");
